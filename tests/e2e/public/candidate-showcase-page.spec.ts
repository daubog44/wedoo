...
test.describe("candidate showcase page", () => {
  test("renders the current candidate showcase shell without the legacy footer", async ({ page }, testInfo) => {
    ...
    await expect(
      layoutRoot.getByRole("heading", { name: publicCopy.candidateShowcase.firstTitle }),
    ).toBeVisible();
  ...
  test("moves to the next slide in the candidate showcase", async ({ page }, testInfo) => {
    const layoutRoot = page.locator(
      `[data-showcase-layout="${testInfo.project.name === "chromium-mobile" ? "mobile" : "desktop"}"]`,
    );
    ...
    await expect(
      layoutRoot.getByRole("heading", { name: publicCopy.candidateShowcase.secondTitle }),
    ).toBeVisible();
  });
  ...
  test("reveals the register CTA on the last desktop slide", async ({ page }, testInfo) => {
    ...
    await expect(
      layoutRoot.getByRole("heading", { name: publicCopy.candidateShowcase.lastTitle }),
    ).toBeVisible();
    await expect(
      layoutRoot.getByRole("link", { name: publicCopy.candidateShowcase.registerCta }),
