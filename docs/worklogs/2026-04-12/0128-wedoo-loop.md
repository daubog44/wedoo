...
- 02:00 - task strict auth recovery acceptance review - node 657:658 / 660:725 / 660:774 / 660:1217 - area recovery/support validation - tests `npx playwright test tests/e2e/public/password-recovery-flow.spec.ts tests/e2e/parity/password-recovery-flow.visual.spec.ts`; `npx vitest run tests/integration/auth-recovery-view-model.test.ts`; `npm run loop:capture -- /password-dimenticata password-recovery-acceptance`; `npm run loop:capture -- /assistenza-clienti customer-support-acceptance` - fresh captures confirmed `/password-dimenticata` remains credible, but `/assistenza-clienti` desktop is still broken in the live route despite green VRT.

- 02:02 - task strict auth recovery acceptance review - node 660:725 - area customer support desktop drift - tests Figma screenshots `660:725`, `660:1217`; capture `artifacts/loop-captures/2026-04-12/0200-customer-support-acceptance/desktop.png` - the purple company canvas is too shallow, company copy clips at the bottom, and the CTA row renders outside the intended shape; this is a concrete real-capture mismatch, so the route must be reopened in `prd.md` and `docs/visual-backlog.md` instead of accepting the green baseline.

- 02:03 - task strict auth recovery acceptance review - node 660:725 - area git/ci surface - tests GitHub branch check - no open PR is currently targeting `codex/ralph-loop-bootstrap`, so there is no PR CI surface to inspect before stopping the round; next action remains the reopened `/assistenza-clienti` frame task.
